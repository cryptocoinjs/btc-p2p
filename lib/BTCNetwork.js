...
BTCNetwork.prototype.parseTxMessage = function parseTxMessage(data, peer) {
  ...
  if (!raw.incrPointer(4)) return false;
  var inCount = raw.readVarInt();
  if (!raw.incrPointer(inCount[1])) return false;
  parsed.txIn = [];
  for (var i = 0; i < inCount[0]; i++) {
    var txIn = { outPoint: {} };
    txIn.outPoint.hash = raw.raw(32);
    if (!raw.incrPointer(32)) return false;
    txIn.outPoint.index = raw.readUInt32LE();
    if (!raw.incrPointer(4)) return false;
    var scriptSize = raw.readVarInt();
    if (!raw.incrPointer(scriptSize[1])) return false;
    txIn.signature = raw.raw(scriptSize[0]);
    if (!raw.incrPointer(scriptSize[0])) return false;
    txIn.sequence = raw.readUInt32LE();
    if (!raw.incrPointer(4)) return false;
    parsed.txIn.push(txIn);
  }
  var outCount = raw.readVarInt();
  if (!raw.incrPointer(outCount[1])) return false;
  parsed.txOut = [];
  for (i = 0; i < outCount[0]; i++) {
    var txOut = {};
    txOut.value = raw.raw(8);
    if (!raw.incrPointer(8)) return false;
    ...
    var scriptSize = raw.readVarInt();
    if (!raw.incrPointer(scriptSize[1])) return false;
    txOut.script = raw.raw(scriptSize[0]);
    if (!raw.incrPointer(scriptSize[0])) return false;
