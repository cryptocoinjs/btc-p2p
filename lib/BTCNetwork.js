...
var MessageBuilder = require('crypto-binary').MessageBuilder;
var MessageParser = require('crypto-binary').MessageParser
...
BTCNetwork.prototype.parseAlertMessage = function parseAlertMessage(data) {
  ...
  var s2 = new MessageParser(message);
...
BTCNetwork.prototype.parseBlockMessage = function parseBlockMessage(data) {
  var s = new MessageParser(data);
  var parsed = {};
  
  parsed.version = s.readUInt32LE();
  parsed.prev_block = s.raw(32);
  parsed.merkle_root = s.raw(32);
  parsed.timestamp = new Date(s.readUInt32LE()*1000);
  parsed.difficulty = s.readUInt32LE();
  parsed.nonce = s.readUInt32LE();
  var txnCount = s.readVarInt();
  parsed.txns = [];
  for (var i = 0; i < txnCount; i++) {
    var tx = this.parseTxMessage(data.slice(s.pointer));
    if (tx === false) return false;
    s.incrPointer(tx.raw.length);
    parsed.txns.push(tx);
  }
...
BTCNetwork.prototype.compileAlert = function compileAlert(data) {
  ...
  var payload = new MessageBuilder(this.options.magic)
