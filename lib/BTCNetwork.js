...
BTCNetwork.prototype.launch = function launch() {
...
BTCNetwork.prototype.handleVersion = function handleVersion(d) {
  ...
  if (parsed.time.readUInt32LE(4) == 0) {
    ...
    this.manager.delActive(d.peer);
    return;
  }
  
  d.peer.send('verack'); // Send VERACK message
  
  if (this.options.externalIP == false || this.options.externalIP.toString('hex') != parsed.addr_recv.hostRaw.toString('hex')) {
