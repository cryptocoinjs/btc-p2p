...
BTCNetwork.prototype.getData = function getData(inventory, peer, callback, force) {
  ...
  // Send the request
  // TODO: Block additional calls to this function until the original request resolves
  
  var toSend = peer.info.pendingGetData;
  ...
  var doCallbacks = function(err, rs) {
    ...
    while (peer.info.getDataListeners.length > 0) {
      var fn = peer.info.getDataListeners.shift(); // Empty the array, as we call them
      fn.call(peer, err, rs);
    }
  };
